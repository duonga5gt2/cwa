...
type ThemeCtx = { theme: "dark" | "light" | string };
type HomeCtx = {
  activeTab: number;
...
export default function HomeTabs() {
  const { theme } = (useTheme() as ThemeCtx);
  const { activeTab, setActiveTab } = (useHome() as HomeCtx);

  const setCookie = (name: string, value: string | number, days = 365) => {
    const d = new Date();
  ...
  const tabGradientsDark: readonly string[] = [
  ...
  const tabGradientsLight: readonly string[] = [
