...
type HomeContextValue = { activeTab: number; setActiveTab: (n: number) => void };
...
function TabsGenerator() {
  ...
  const regenTab = (it: TabItem) => tabsForm(it.id, it.name);
  const regenSection = (it: TabItem, idx: number) =>
    sectionForm(it.id, it.title, it.content, idx === 0 ? 0 : 1);

  const syncFromItems = (list: TabItem[]) => {
  ...
  const focusRing = (c = tone.primary) =>
    `0 0 0 3px ${isDark ? "rgba(34,211,238,0.25)" : "rgba(14,165,233,0.25)"}, 0 1px 1px rgba(0,0,0,0.05)`;

  const tabsForm = (id: string, name: string): string => {
...
${name}</button>`;
  ...
  const sectionForm = (id: string, title: string, content: string, index: number): string => {
    const isFirst = index === 0;
    const finalForm = `<section id="panel-${id}"
             role="tabpanel"
             aria-labelledby="tab-${id}"
             style="${isFirst ? "" : "display:none;"}border:1px solid ${tone.border};
  ...
  const addTabsAndSections = (idV: string, nameV: string, titleV: string, contentV: string) => {
    if (!(idV && nameV && titleV && contentV)) return;
    const nextItems = [...items, { id: idV, name: nameV, title: titleV, content: contentV }];
    setItems(nextItems);
  ...
  const generateFullHTMLPage = (): string => {
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
<body style="font-family:${fontSans};margin:16px;background:${tone.bg};color:${tone.text}">
  <div role="tablist" aria-label="Pages"
       style="display:flex;gap:8px;border-bottom:1px solid ${tone.border};margin:0 0 12px 0;padding-bottom:6px;">
    ${tabs.join("\n")}
  </div>
  ${sections.join("\n")}
  <script>
    function openTab(id){
      ...
      document.querySelectorAll('[role="tab"]').forEach(b=>{
        ...
        b.style.boxShadow="inset 0 -1px 0 ${isDark ? "#00000033" : "#0000000a"}";
      });
  ...
  const updateItem = (index: number, patch: Partial<TabItem>) => {
  ...
  const cardBaseStyle: React.CSSProperties = {
    ...
    fontFamily: fontSans,
    border: `1px solid ${tone.border}`,
    boxShadow: isDark ? "0 8px 28px rgba(0,0,0,.45)" : "0 10px 30px rgba(15,23,42,.08)",
  ...
  const labelHint: React.CSSProperties = { fontSize: "12px", opacity: 0.8, color: tone.textMuted };
  ...
  return (
    <div
      className={clsx(theme === "dark" ? "bg-black" : "bg-white", "min-h-screen")}
      style={{
        background: isDark
    ...
    >
      ...
      <div className="flex flex-wrap justify-evenly items-center" style={{ padding: "16px", gap: 16 }}>
        <div>
          <div style={{ ...cardBaseStyle, width: "420px", display: "grid", gap: "12px" }}>
            <label>
              ...
              <input
                ...
                onBlur={(e) => (e.currentTarget.style.boxShadow = "inset 0 1px 2px rgba(0,0,0,.05)")}
                style={inputBase}
              />
            </label>
            ...
            <label>
              ...
              <textarea
                ...
                onFocus={(e) => (e.currentTarget.style.boxShadow = focusRing())}
                onBlur={(e) => (e.currentTarget.style.boxShadow = "inset 0 1px 2px rgba(0,0,0,.05)")}
                style={{ ...inputBase, resize: "vertical", minHeight: 120, fontFamily: fontMono }}
              />
            </label>
            ...
            items.map((it, i) => (
              <div
                ...
                style={{
                  ...
                  boxShadow: isDark ? "0 1px 0 rgba(0,0,0,0.4)" : "0 1px 0 rgba(16,24,40,0.04)",
                  marginBottom: 8,
                }}
              >
                <div style={{ display: "grid", gap: 6 }}>
                  ...
                  <label style={{ display: "grid", gap: 4 }}>
                    <span style={{ ...labelHint, fontSize: 11 }}>Content (HTML ok)</span>
                    <textarea
                      value={it.content}
                      rows={3}
                      ...
                      onFocus={(e) => (e.currentTarget.style.boxShadow = focusRing())}
                      onBlur={(e) => (e.currentTarget.style.boxShadow = "inset 0 1px 2px rgba(0,0,0,.05)")}
                      style={{ ...inputBase, resize: "vertical", minHeight: 80, fontFamily: fontMono }}
                    />
                  </label>
                </div>
        ...
        <div style={{ ...cardBaseStyle, width: "540px" }}>
          ...
          <button
            ...
            onMouseDown={(e) => (e.currentTarget.style.transform = "translateY(1px)")}
            onMouseUp={(e) => (e.currentTarget.style.transform = "translateY(0)")}
            onClick={() => navigator.clipboard.writeText(generateFullHTMLPage())}
            style={{ ...primaryBtn, display: "flex", alignItems: "center", gap: "8px", fontFamily: fontMono, fontWeight: 700 }}
          >
...
function CodingRaces() {
  ...
  return (
    ...
    >
      ...
      <p style={{ color: isDark ? "#a6adbb" : "#475569" }}>Not yet finished. Add Coding Races content here.</p>
    </div>
  );
}
...
function CourtRoom() {
  ...
  return (
    ...
    >
      ...
      <p style={{ color: isDark ? "#a6adbb" : "#475569" }}>Not yet finished. Put your Court Room content here.</p>
    </div>
  );
}

export default function Home() {
  const { activeTab } = (useHome() as HomeContextValue);
  const { theme } = (useTheme() as ThemeContextValue);
  const isDark = theme === "dark";
